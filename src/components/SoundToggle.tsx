...
import { isSoundEnabled, subscribeSound, toggleSoundEnabled } from '../utils'
...
function SpeakerOnIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="sound-toggle__icon">
      <path
        fill="currentColor"
...
export default function SoundToggle({ className = '' }: Props) {
  ...
  return (
    <button
      type="button"
      className={`sound-toggle ${enabled ? 'sound-toggle--on' : 'sound-toggle--off'}${className ? ` ${className}` : ''}`}
